...
export class Migration20260528144300 extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`alter table if exists "buyer_account_status" drop constraint if exists "buyer_account_status_customer_id_unique";`);
    ...
    this.addSql(`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_buyer_account_status_customer_id_unique" ON "buyer_account_status" ("customer_id") WHERE deleted_at IS NULL;`);
    ...
    this.addSql(`create table if not exists "refusal_dispute" ("id" text not null, "order_id" text not null, "dispatch_order_id" text not null, "customer_id" text not null, "rider_id" text null, "rider_photo_url" text null, "rider_notes" text null, "buyer_reason" text check ("buyer_reason" in ('damaged_goods', 'wrong_item', 'not_home', 'other')) null, "buyer_notes" text null, "buyer_responded_at" timestamptz null, "producer_response" text null, "producer_responded_at" timestamptz null, "resolution" text check ("resolution" in ('pending', 'buyer_fault', 'producer_fault', 'rider_fault', 'platform_fault')) not null default 'pending', "resolution_notes" text null, "resolved_by" text null, "resolved_at" timestamptz null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "refusal_dispute_pkey" primary key ("id"));`);
