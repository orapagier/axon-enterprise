/**
 * Stackable account roles (2026-06 reframe).
 *
 ...
 * Trader are mutually exclusive (founder call — traders buy B2B, they never
 * sell). Accounts created before the reframe carry a single `account_type`
 * instead and are treated as the equivalent one-role set — but only until a
 * `roles` array is written. Once `roles` exists it is authoritative, so a
 * downgrade can't be undone by the stale legacy field.
 */
...
export function rolesOf(meta: Record<string, unknown>): StackableRole[] {
...
export function hasRole(meta: Record<string, unknown>, role: Role): boolean {
