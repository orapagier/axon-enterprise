...
const options = [
  {
    ...
    desc: "You pack and deliver this order yourself. FreshHub charges only the membership fee — no transaction commission.",
  },
  {
    value: "sell_to_freshhub",
    icon: "🏭",
    label: "Sell to FreshHub",
    desc: "Bring your harvest to the hub at a scheduled pickup window. FreshHub handles dispatch and delivery to consumers.",
  },
] as const
    ...
    <div className="px-7 small:px-12 py-5 border-b border-grey-10 bg-grey-5/30">
      ...
      {disabled && (
        <div className="mt-2 text-[11px] text-amber-700 bg-amber-50 px-3 py-2 rounded-lg border border-amber-200">
          Listing type is locked once the listing is no longer a draft.
        </div>
      )}
    </div>
  )
